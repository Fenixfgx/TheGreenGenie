...
async function fetchData(client, date) {
  const url = `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/${SHEET_NAME}!F:AT?key=${API_KEY}`;
...
function displayResults(rows, client, date) {
  const filteredRows = rows.filter(row => row[0] === client && row[40] === date);
